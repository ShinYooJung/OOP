...
Number *Integer::add(Number * num) {
	...
	int sum = this->val() + ((Integer*) num)->val();
	((Integer*) num)->set_val(sum);
	delete(this);
	return num;

    } else if (num->types() == FLOAT) {
	double sum = this->val() + ((Float*) num)->val();
	((Float*) num)->set_val(sum);
	delete(this);
	return num;

    } else if (num->types() == COMPLEX) {
 	double sum = this->val() + ((Complex*) num)->real();
        ((Complex*) num)->set_val(sum, ((Complex*) num)->imag());
        delete(this);
        return num;
...
Number *Integer::mul(Number * num) {
     ...
     */
     if (num->types() == INTEGER) {
        ...
        double sum1 = this->val() * ((Complex*) num)->real();
	double sum2 = this->val() * ((Complex*) num)->imag();
        ((Complex*) num)->set_val(sum1, sum2);
        delete(this);
        return num;
